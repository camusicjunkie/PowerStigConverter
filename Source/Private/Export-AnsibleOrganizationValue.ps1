...
'@ -f (New-AnsibleVariable -StigName $StigName -Type OrganizationValueGroup) | Out-File -FilePath $PSScriptRoot\Roles\main_default_org.yml

        if ($organization.Count -gt 0) { $organization.Values | Out-File -FilePath $PSScriptRoot\Roles\main_default_org.yml -Append }
    }
}
